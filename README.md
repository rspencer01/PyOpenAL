...
python-openal requires a dynamic OpenAL library (OpenAL32.dll). 
...
python-openal provides OpenAL bindings, as you would find them in C++, meaning you can follow any [OpenAL C++ tutorial](http://www.openal.org/documentation/) with Python.
...
Don't worry though, python-openal can be used without the need to do that.
...
##### Playing a file
	...
	# python-openal does that for us (if it can find it)
...
### Reference (for python-openal's own classes and functions)

	<method> oalInit(device_specifier = None, context_attr_list = None) -> None
		# initializes python-openal
	...
	<method> oalGetInit() -> <bool> initialized
		# finds out wether or not python-openal is initialized
		
	<method> oalGetContext() -> <int> context
		# returns the context used by python-openal
		
	<method> oalGetDevice() -> <int> device
		# returns the device used by python-openal
		
	<method> oalSetAutoInit(val) -> None
		# changes wether or not python-openal initializes automatically
	...
	<class> openal.Listener()
		...
		# an instance of this class is created when you import python-openal.
	...
	<method> openal.oalGetListener() -> Listener
		# returns python-openal's Listener
